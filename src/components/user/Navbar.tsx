...
import { XIcon, AlignJustify, DownloadCloudIcon } from 'lucide-react';
import DownloadsPage from '@/app/(user)/downloads/page';
  ...
  return (
    <nav className="flex mx-auto items-center justify-between gap-2 py-5 px-8 w-full xl:w-[1200px]">
      ...
      {isOpen && (
        <div
          className={
            'bg-white fixed dark:bg-[#090a22] z-10 py-5 px-10 w-full top-0 left-0 h-screen gap-7 md:hidden'
          }
        >
      ...
      <div className="flex gap-3 items-center">
        <SignedOut>
          <SignInButton>
            <CustomButton className="btn-signin" rounded={'full'} size={'sm'}>
              Sign In
            </CustomButton>
          </SignInButton>
        </SignedOut>
        ...
        <SignedIn>
          <UserButton afterSignOutUrl="/">
            <UserButton.UserProfilePage
              label="Downloads"
              url="/downloads"
              labelIcon={<DownloadCloudIcon className="size-4" />}
            >
              <DownloadsPage />
            </UserButton.UserProfilePage>
          </UserButton>
